...
const handleGoogleCallback = async (accessToken, _, tokenDetail, profile, done) => {
  ...
  User.findOne({ email, connectGoogle: true }).then(existingUser => {
    if (existingUser) {
      ...
      User.findOne({ email }).then(user => {
        if (!user) {
          ...
          User.updateOne({ email }, { connectGoogle: true })
...
const handleFacebookCallback = async (accessToken, _, tokenDetail, profile, done) => {
  const email = profile.emails[0].value;
  User.findOne({ email, connectFacebook: true }).then(existingUser => {
    if (existingUser) {
      done(null, existingUser);
    } else {
      User.findOne({ email }).then(user => {
        if (!user) {
          new User({ email, connectFacebook: true }).save()
            .then(newUser => { done(null, newUser); });
        } else {
          User.updateOne({ email }, { connectFacebook: true })
            .then(updatedUser => { done(null, updatedUser); });
        }
      });
    }
